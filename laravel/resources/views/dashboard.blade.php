...
<div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8 space-y-8">
    ...
    <div class="bg-gray-800 rounded-xl p-6 border border-gray-700 shadow-lg flex flex-col md:flex-row gap-8 items-start">
        <div class="flex flex-col items-center md:items-start text-center md:text-left min-w-[200px]">
            @if($user->avatar_url)
                <img src="{{ asset('storage/' . $user->avatar_url) }}" class="h-24 w-24 rounded-full object-cover border-4 border-gray-600 mb-4 shadow-lg">
            @else
                            ...
                            new Chart(document.getElementById('racePosChart'), {
                                ...
                                options: {
                                    ...
                                    scales: {
                                        ...
                                        x: { grid: { display: false }, ticks: { color: '#9ca3af' } } // Etiquetas X visibles
                                    }
                                }
                            });
                        }
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            ...
                            new Chart(document.getElementById('pointsChart'), {
                                ...
                                data: {
                                    ...
                                    datasets: [{
                                        label: 'Total',
                                        data: this.data,
                                        borderColor: '#ef4444', // <--- AHORA ES ROJO
                                        backgroundColor: 'rgba(239, 68, 68, 0.1)', // Rojo transparente
                                        borderWidth: 3,
                                        pointRadius: 5, // Puntos grandes
                                        pointBackgroundColor: '#1f2937', // Punto negro con borde rojo
        ...
        <!-- (La tabla de Qualy sigue aquí igual...) -->

        <!-- ABAJO DER: Tabla Histórica Qualy (RECUPERADA) -->
    ...
    <!-- 5. ABAJO DEL TODO: TABLA REPORTES -->
                ...
                <tbody class="divide-y divide-gray-700">
                    @forelse($myReports as $report)
                        <tr class="hover:bg-gray-700/30">
                            ...
                            <td class="px-6 py-4">
                                @if($report->reporter_id === Auth::id()) <span class="text-blue-400 font-bold">You Reported</span> @else <span class="text-red-400 font-bold">Reported You</span> @endif
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-8 text-center text-gray-500 italic">Clean record.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
